...
# pandas:常用IO
# df5_1 = pd.DataFrame((np.arange(8)).reshape(2,4))
# df5_1.to_csv('1.csv') # 根据dataframe创建csv文件
# df5_1.to_csv('D:/1.csv') #在其他路径下创建文件
# df5_2 = pd.read_csv('1.csv') # 读取csv文件为dataframe格式
# df5_3 = pd.read_csv(r'D:/1.csv') # 读取其他路径的文件
# print(df5_2, df5_3)
# 其他类型文件的读取采用类似的形式

# pandas：dataframe的合并
# df6 = pd.DataFrame(np.arange(12).reshape(3, 4))
# df7 = pd.DataFrame(np.arange(13, 25).reshape(3, 4))
# df8 = pd.DataFrame(np.arange(25, 37).reshape(3, 4))
# print(pd.concat((df6, df7, df8), axis=0, ignore_index=True)) #pd.concat合并多个dataframe，可以定义合并的方向
# df9 = pd.DataFrame(np.arange(12).reshape(3, 4), columns=['a', 'b', 'c', 'd'], index=[1,2,3])
# df10 = pd.DataFrame(np.arange(13, 25).reshape(3, 4), columns=['c', 'd', 'e', 'f'], index=[1,2,3])
# print(df9)
# print(df10)
# print(pd.concat((df9, df10), axis=0, join='outer', ignore_index=False))  # 合并时label不同可以选择合并的模式，outer表示取并
# print(pd.concat((df9, df10), axis=0, join='inner', ignore_index=True))  # inner表示取交
# print(pd.concat((df9, df10), axis=1, join='outer')) # 选择合并的方向
# print(df9.append(df10)) # 在行列上均合并，注意label名称只对列有效，行是分开的，即使index相同也无法合并在一起
# print(pd.DataFrame(np.arange(3)))
