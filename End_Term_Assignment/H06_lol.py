...
class Vertex:
    # 创建演员代表的节点，包含属性：演员名称；演员的合作者（连接的节点，字典，key为合作者顶点，value为合作电影的列表）；演员出演的电影列表
    def __init__(self, name, movie, type, star):
        self.name = name
        self.collaborators = {}
        self.movies = {movie: (type, star)}
        self.searched = False
        self.distance = 0

    def addMovie(self, movie, type, star):  # 添加演员所出演的电影进入出演电影列表
        self.movies[movie] = (type, star)

    def addCollaborator(self, nbr, commom, type, star):  # 新增合作者
        self.collaborators[nbr] = {commom: (type, star)}

    def addCommon(self, nbr, common, type, star):  # 原有合作者，新增合作电影
        self.collaborators[nbr][common] = (type, star)
...
class Graph:  # 基于邻接列表的图模型
    ...
    def addVertex(self, name, movie, type, star):
        self.numVertices += 1
        newVertex = Vertex(name, movie, type, star)
    ...
    for movieInfo in lst:
        ...
        for pos in range(len(actLst)):
            ...
            if actor not in g:  # 如果是图中没有的新演员则将其作为新的顶点加入
                g.addVertex(actor, title, type, star)
                # 对于后方的其他演员进行遍历，这样减少了操作次数，如果对每个演员都对其他所有演员遍历的话计算量较大
                for num in range(pos + 1, len(actLst)):
                    addCo(actor, actLst[num], title, type, star, g)
            else:  # 如果是已经在图中的演员
                g.getVertex(actor).addMovie(title, type, star)
                for num in range(pos + 1, len(actLst)):
                    addCo(actor, actLst[num], title, type, star, g)
    return g


def addCo(actor, co, title, type, star, graph):  # 添加合作者或者已有合作者之间的合作电影
    coVert = graph.getVertex(co) if co in graph else graph.addVertex(co, title, type, star)
    if coVert not in graph.vertList[actor].getCollaborators():  # 如果是新的合作者，则创建新的连接
        graph.vertList[actor].addCollaborator(coVert, title, type, star)
        graph.vertList[co].addCollaborator(graph.vertList[actor], title, type, star)
    else:  # 如果是原有的合作者,则在原合作电影列表中添加此电影
        graph.vertList[actor].addCommon(coVert, title, type, star)
        graph.vertList[co].addCommon(graph.vertList[actor], title, type, star)
...
def bfs(vert, bo):
    ...
    while len(vertQueue) > 0:
        ...
        for co in currentVert.collaborators.keys():
            if co.searched == bo:  # 由于之前进行过dfs，因此所有节点现在的搜索状态都是True，此处把True看成未搜索就行了
                co.setStatus(not bo)
...
# 计算一节点所在连通分支的直径
def diameter(vert):
    last = bfs(vert, True)[1]  # 从任意一节点出发进行广度优先搜索，得到的最远节点一定是直径对应的一端
    dia = bfs(last, False)[0]  # 再次进行bfs搜索，即可得到连通分支的直径
    return dia
...
size = [i[0] for i in infoLst]
dia = [i[2] for i in infoLst]
star = [i[3] for i in infoLst]

plt.rcParams['font.sans-serif']=['Microsoft YaHei']
fig = plt.figure(figsize=(10,4))
print('各连通分支的大小如图所示')
bax = brokenaxes(xlims=((0, 21), (4556, 4578)), ylims=((0, 50), (84682, 84692)))
bax.bar(X, size, facecolor='midnightblue')
plt.savefig('size.png')
plt.show()
fig = plt.figure(figsize=(10,4))
print('各连通分支的直径如图所示')
bax = brokenaxes(xlims=((0, 21), (4556, 4578)))
bax.bar(X, dia, facecolor='midnightblue')
plt.savefig('diameter.png')
plt.show()
fig = plt.figure(figsize=(10,4))
print('各连通分支的电影平均星级如图所示')
bax = brokenaxes(xlims=((0, 21), (4556, 4578)))
bax.bar(X, star, facecolor='midnightblue')
...
actorLst1 = ['张子枫','赵丽颖','郑爽','景甜','迪丽热巴','关晓彤','杨颖','古力娜扎','鹿晗', '陈学冬', '吴亦凡', '王源', '易烊千玺', '王俊凯', '柯震东', '黄子韬','肖战']
actorLst2 = ['章子怡','周迅','赵薇','刘亦菲','巩俐','高圆圆','黄圣依','姚晨','徐峥','姜文','冯小刚','葛优','周星驰','黄渤','梁朝伟','甄子丹','李连杰','陈道明','孙红雷']
aveLst1, aveLst2 = [], []
numLst1, numLst2 = [], []
for actor in actorLst1:
    ...
    print(actor+'出演了'+str(len(mov))+'部电影,主要是'+str(typeLst[0][0])+'类型的电影,平均星级为'+str(aveStar)+',最高星级为'+str(max(starLst))+',最低星级为'+str(min(starLst)))
print('***以上新生代演员出演的电影的平均数目为'+str(round(sum(numLst1)/len(numLst1)))+',平均星级为'+str(round(sum(aveLst1)/len(actorLst1),2)))
for actor in actorLst2:
    ...
    print(actor+'出演了'+str(len(mov))+'部电影,主要是'+str(typeLst[0][0])+'类型的电影,平均星级为'+str(aveStar)+',最高星级为'+str(max(starLst))+',最低星级为'+str(min(starLst)))
print('***以上老一辈演员出演的电影的平均数目为'+str(round(sum(numLst2)/len(numLst2)))+',平均星级为' + str(round(sum(aveLst2) / len(actorLst1), 2)))